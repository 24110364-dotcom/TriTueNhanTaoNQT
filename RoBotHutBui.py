...
def sensorless_search(start_state):
    ...
    for r in range(3):
        for c in range(3):
            if current[r][c] == 'x1' or current[r][c] == 1:
                current[r][c] = 1
            else:
                current[r][c] = 0
    ...
    r, c = 2, 2
    current[r][c] = 'x1' if current[r][c] == 1 else 'x'
    ...
    for action in fixed_actions:
        ...
        if action == "SUCK":
            ...
            current[r][c] = 1 if current[r][c] == 'x1' else 0
            # Di chuyển robot
            if action == "UP" and r > 0: r -= 1
            elif action == "DOWN" and r < 2: r += 1
            elif action == "LEFT" and c > 0: c -= 1
            elif action == "RIGHT" and c < 2: c += 1
...
def partial_sensing_search(start_state):
    ...
    while not is_goal(current):
        ...
        if "SUCK" in actions:
            current = result_state(current, "SUCK")
            path.append(current)
        ...
        for action in actions:
            ...
            r_next, c_next = find_robot(child)
            if current[r_next][c_next] == '1' or current[r_next][c_next] == 'x1':
                if child_tuple not in reached:
                    next_step = child
                    break
            else:
                if child_tuple not in reached:
                    better_actions.append(child)
                    
        
        if next_step is None and better_actions:
            next_step = random.choice(better_actions)
            
        
        if next_step is None and actions:
            random_action = random.choice(actions)
            next_step = result_state(current, random_action)
